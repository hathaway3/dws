...
public class DWVModem {
	...
	private int doCommand(String thiscmd, String thisreg, String thisarg) {
		...
		logger.debug("vmodem doCommand: " + thiscmd + "  reg: " + thisreg + " (" + regval + ")  arg: " + thisarg + " ("
				+ val + ")");
		...
		case 'I':
			...
			case 1:
			case 3:
				write(getCRLF() + "DriveWire virtual modem on port " + dwVSerialPorts.prettyPort(this.vport)
						+ getCRLF());
				break;
			case 2:
				try {
					if (dwVSerialPorts.getConn(this.vport) > -1)
						write(getCRLF() + "Connected to " + dwVSerialPorts.getHostIP(this.vport) + ":"
								+ dwVSerialPorts.getHostPort(this.vport) + getCRLF());
					else
	...
	private String getCRLF() {
		return (Character.toString((char) this.vmodem_registers[REG_CR])
				+ Character.toString((char) this.vmodem_registers[REG_LF]));
	}
