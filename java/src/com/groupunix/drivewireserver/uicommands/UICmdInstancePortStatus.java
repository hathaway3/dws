	...
	public DWCommandResponse parse(String cmdline) {
		...
		if (this.gproto == null) {
			if ((DriveWireServer.isValidHandlerNo(this.dwuithread.getInstance())
					&& (DriveWireServer.getHandler(this.dwuithread.getInstance()).hasVSerial()))) {
		...
		/*
		 * Iterator<DWUIClientThread> itr =
		 * DriveWireServer.getDWUIThread().getUIClientThreads().iterator();
		 * 
		 * while(itr.hasNext()) { DWUIClientThread client = itr.next();
		 * 
		 * // filter for instance if ((client.getInstance() == -1) ||
		 * (client.getInstance() == this.dwuithread.getInstance())) { res += "U|" +
		 * client.getInstance() + "|" + client.getState() + "|" + client.getCurCmd() +
		 * "|" + client.getSocket().getInetAddress().getHostAddress() + "|" +
		 * client.getSocket().getPort(); res += "\r\n";
		 * 
		 * } }
		 */
