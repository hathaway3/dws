...
public class DWDECBFileSystem extends DWFileSystem {
	...
	public ArrayList<DWDiskSector> getFileSectors(String filename)
			throws DWFileSystemFileNotFoundException, DWFileSystemInvalidFATException, IOException,
			DWDiskInvalidSectorNumber, DWFileSystemInvalidDirectoryException {

		return (getFAT().getFileSectors(disk.getSectors(),
				((DWDECBFileSystemDirEntry) getDirEntry(filename)).getFirstGranule()));

	}

	public DWFileSystemDirEntry getDirEntry(String filename)
			throws DWFileSystemFileNotFoundException, IOException, DWFileSystemInvalidDirectoryException {
	...
	public byte[] getFileContents(String filename)
		...
		if (bl > 0) {
			System.arraycopy(sectors.get(sectors.size() - 1).getData(), 0, res,
	...
	public void addFile(String filename, byte[] filecontents) throws DWFileSystemFullException,
			DWFileSystemInvalidFilenameException, DWFileSystemFileNotFoundException, DWFileSystemInvalidFATException,
			IOException, DWDiskInvalidSectorNumber, DWFileSystemInvalidDirectoryException {
	...
	private void addDirectoryEntry(String filename, byte firstgran, byte leftovers)
			throws DWFileSystemFullException, DWFileSystemInvalidFilenameException, IOException,
			DWDiskInvalidSectorNumber, DWFileSystemInvalidDirectoryException {
		...
		DWDECBFileSystemDirExtensionMapping mapping = new DWDECBFileSystemDirExtensionMapping(ext, DECBDefs.FLAG_BIN,
				DECBDefs.FILETYPE_ML);

		if (DriveWireServer.getConfig().getMaxIndex("DECBExtensionMapping") > -1) {
			for (int i = 0; i <= DriveWireServer.getConfig().getMaxIndex("DECBExtensionMapping"); i++) {
				String kp = "DECBExtensionMapping(" + i + ")";
				// validate entry first
				if (DriveWireServer.getConfig().containsKey(kp + "[@extension]")
						&& DriveWireServer.getConfig().containsKey(kp + "[@ascii]")
						&& DriveWireServer.getConfig().containsKey(kp + "[@filetype]")) {
					if (DriveWireServer.getConfig().getString(kp + "[@extension]").equalsIgnoreCase(ext)) {
						// we have a winner

						mapping.setType(DriveWireServer.getConfig().getByte(kp + "[@filetype]"));

						if (DriveWireServer.getConfig().getBoolean(kp + "[@ascii]"))
	...
	public void format() throws DWInvalidSectorException, DWSeekPastEndOfDeviceException,
			DWDriveWriteProtectedException, IOException {
	 ...
	 * {
	 * this.fserrors.add(e.getFileName() + "." + e.getFileExt() +
	 * ": First granule of " + e.getFirstGranule() + " is > FAT size");
	 * wacky = true;
	 * }
	 * // Some applications use their own filetypes, so remove this.. found .MUS =
	 * type 4
	 * //else if (e.getFileType() > 3)
	 * //{
	 * // System.out.println(e.getFileName() + "." + e.getFileExt() +
	 * ": FileType of " + e.getFileType() + " is > 3..?");
	 * // wacky = true;
	 * //}
	 * else if ( (e.getFileFlag() != 0) && (e.getFileFlag() != 255) )
	 * {
	 * this.fserrors.add(e.getFileName() + "." + e.getFileExt() + ": FileFlag of " +
	 * e.getFileFlag() + " is not defined..?");
	 * wacky = true;
	 * }
	 * }
	 * 
	 ...
	 * 
	 * if (((val > DECBDefs.FAT_SIZE) && (val < 0xC0)) || ((val > 0xCF) && (val <
	 * 0xFF)))
	 * {
	 * this.fserrors.add("FAT entry #"+ i + " is " + val +
	 * ", which points beyond FAT");
	 * wacky = true;
	 * }
	 * }
	 ...
	 * else
	 * {
	 * this.fserrors.add("Disk size of " + this.disk.getDiskSectors() +
	 * " doesn't match known DECB image size");
	 * }
	 * 
	 * return false;
	 * }
			...
			try {
				...
				for (DWFileSystemDirEntry e : dir) {
					...
					if (((DWDECBFileSystemDirEntry) e).getFirstGranule() > DECBDefs.FAT_SIZE) {
						this.fserrors.add(e.getFileName() + "." + e.getFileExt() + ": First granule of "
								+ ((DWDECBFileSystemDirEntry) e).getFirstGranule() + " is > FAT size");
						wacky = true;
					}
					// Some applications use their own filetypes, so remove this.. found .MUS = type
					// 4
					// else if (e.getFileType() > 3)
					// {
					// System.out.println(e.getFileName() + "." + e.getFileExt() + ": FileType of "
					// + e.getFileType() + " is > 3..?");
					// wacky = true;
					// }
					else if ((((DWDECBFileSystemDirEntry) e).getFileFlag() != 0)
							&& (((DWDECBFileSystemDirEntry) e).getFileFlag() != 255)) {
						this.fserrors.add(e.getFileName() + "." + e.getFileExt() + ": FileFlag of "
								+ ((DWDECBFileSystemDirEntry) e).getFileFlag() + " is not defined..?");
						wacky = true;
					}
				}
