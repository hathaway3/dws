...
public class DWDiskLazyWriter implements Runnable {
	...
	public void run() {
		...
		logger.debug("started, write interval is " + DriveWireServer.getDiskLazyWriteInterval(2000));

		while (wanttodie == false) {

			try {
				// logger.debug("sleeping for " +
				// DriveWireServer.serverconfig.getLong("DiskLazyWriteInterval",15000) + "
				// ms...");
				Thread.sleep(DriveWireServer.getDiskLazyWriteInterval(5000));
				syncDisks();
