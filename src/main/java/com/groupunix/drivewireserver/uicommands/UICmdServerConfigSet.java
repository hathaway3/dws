...
public class UICmdServerConfigSet extends DWCommand {
	...
	private DWCommandResponse doSetConfig(String item) {
		...
		if (DriveWireServer.getConfig().containsKey(item)) {
			synchronized (DriveWireServer.getConfig()) {
				DriveWireServer.getConfig().setProperty(item, null);
			}
	...
	private DWCommandResponse doSetConfig(String item, String value) {
		synchronized (DriveWireServer.getConfig()) {
			if (DriveWireServer.getConfig().containsKey(item)) {
				if (!DriveWireServer.getConfig().getProperty(item).equals(value))
					DriveWireServer.getConfig().setProperty(item, value);
			} else {
				DriveWireServer.getConfig().setProperty(item, value);
			}
		}
