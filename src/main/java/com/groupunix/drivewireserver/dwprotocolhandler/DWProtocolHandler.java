...
public class DWProtocolHandler implements DWVSerialProtocol {
	...
	public void run() {
		...
		// this thread should run a LOT or we might lose bytes on the serial port on
		// slow computers
			...
			while (!wanttodie) {
				...
				if (!(protodev == null) && !resetPending) {
					try {
						...
						// this should not actually ever get thrown, since we call comRead1 with timeout
						// = false..
				...
				if ((opcodeint > -1) && (this.protodev != null)) {
					...
					if (this.protodev.getClass().getCanonicalName()
							.equals("com.groupunix.drivewireserver.dwprotocolhandler.DWSerialDevice"))
					...
					try {
						...
						if ((lastOpcode >= DWDefs.OP_FASTWRITE_BASE)
								&& (lastOpcode <= (DWDefs.OP_FASTWRITE_BASE + this.dwVSerialPorts.getMaxPorts() - 1))) {
						...
						logger.warn(
								"Invalid port # from CoCo in " + DWUtils.prettyOP(lastOpcode) + ": " + e.getMessage());
					}
					...
					if (optook > DWDefs.SERVER_SLOW_OP)
						...
						logger.debug(DWUtils.prettyOP(lastOpcode) + " took " + optook + "ms, serial read delay was "
					...
					if (!this.wanttodie) {
						...
						if (this.resetPending) {
							...
							logger.debug("device unavailable, will retry in "
									+ config.getInt("DeviceFailRetryTime", 6000) + "ms");
	...
	private void DoOP_WRITE(byte opcode) throws DWCommTimeOutException, IOException {
		...
		if (lastChecksum != DWUtils.int2(cocosum)) {
			...
			logger.warn("DoOP_WRITE: Bad checksum, drive: " + lastDrive + " LSN: " + DWUtils.int3(lastLSN)
					+ " CocoSum: " + DWUtils.int2(cocosum) + " ServerSum: " + lastChecksum);
	...
	private void DoOP_READ(int opcode) throws IOException, DWCommTimeOutException {
		...
		try {
			...
			// attempt seek to requested LSN (will throw one of the many exceptions caught
			// below if it cannot)
			diskDrives.seekSector(lastDrive, DWUtils.int3(lastLSN));

			// we didn't throw an exception in our seek, so load a buffer with the sector's
			// data
			sector = diskDrives.readSector(lastDrive);

			// deal with all kinds of things that could have gone wrong as we did the seek
			// and read above...
		} catch (DWDriveNotLoadedException e1) {
	...
	private void DoOP_READEX(int opcode) throws IOException, DWCommTimeOutException {
		...
		if (!config.getBoolean("ProtocolDisableReadChecksum", false)) {
			...
			if (((mysum[0] == cocosum[0]) && (mysum[1] == cocosum[1]))
				...
				if (opcode == DWDefs.OP_REREADEX) {
					readRetries++;
					logger.warn("DoOP_REREADEX CRC check failed, lastDrive: " + lastDrive + " LSN: "
							+ DWUtils.int3(lastLSN));
				} else {
					logger.warn(
							"DoOP_READEX CRC check failed, lastDrive: " + lastDrive + " LSN: " + DWUtils.int3(lastLSN));
	...
	private void DoOP_STAT(byte opcode) throws IOException, DWCommTimeOutException {
		...
		if (opcode == DWDefs.OP_GETSTAT) {
			...
			if (config.getBoolean("LogOpCode", false)) {
				logger.info("DoOP_GETSTAT: " + DWUtils.prettySS(responsebuf[1]) + " lastDrive: " + lastDrive + " LSN: "
						+ DWUtils.int3(lastLSN));
			}
		} else {
			lastSetStat = responsebuf[1];
			if (config.getBoolean("LogOpCode", false)) {
				logger.info("DoOP_SETSTAT " + DWUtils.prettySS(responsebuf[1]) + " lastDrive: " + lastDrive + " LSN: "
						+ DWUtils.int3(lastLSN));
			}
		}
	}
	...
	private void DoOP_SERGETSTAT() throws IOException, DWCommTimeOutException {
		...
		// Z
		if ((responsebuf[0] >= this.dwVSerialPorts.getMaxNPorts())
				&& (responsebuf[0] < this.dwVSerialPorts.getMaxZPorts())) {
		...
		{
			if (config.getBoolean("LogOpCode", false)) {
				logger.info("DoOP_SERGETSTAT: " + DWUtils.prettySS(responsebuf[1]) + " port: " + responsebuf[0] + "("
						+ dwVSerialPorts.prettyPort(responsebuf[0]) + ")");
			}
		}
	...
	private void DoOP_SERSETSTAT() throws IOException, DWCommTimeOutException {
		...
		try {
			...
			if (config.getBoolean("LogOpCode", false)) {
				logger.info("DoOP_SERSETSTAT: " + DWUtils.prettySS(responsebuf[1]) + " port: " + responsebuf[0] + "("
						+ dwVSerialPorts.prettyPort(responsebuf[0]) + ")");
			}
	...
	private void setupProtocolDevice() {
		...
		if (config.getString("DeviceType", "dummy").equalsIgnoreCase("dummy")) {
			...
			if ((config.containsKey("SerialDevice") && config.containsKey("SerialRate"))) {
				...
				logger.error(
						"Serial mode requires both SerialDevice and SerialRate to be set, please configure this instance.");
				// wanttodie = true;
			}
			...
			if (config.containsKey("TCPClientPort") && config.containsKey("TCPClientHost")) {
				try {
					protodev = new DWTCPClientDevice(this.handlerno, config.getString("TCPClientHost"),
				...
				logger.error(
						"TCP client mode requires TCPClientPort and TCPClientHost to be set, cannot use this configuration");
				// wanttodie = true;
			}
	...
	public String getStatusText() {
		...
		// text += "Total Read Sectors: " +
		// String.format("%6d",DriveWireServer.getHandler(handlerno).getSectorsRead()) +
		// " (" + DriveWireServer.getHandler(handlerno).getReadRetries() + "
		// retries)\r\n";
		// text += "Total Write Sectors: " +
		// String.format("%6d",DriveWireServer.getHandler(handlerno).getSectorsWritten())
		// + " (" + DriveWireServer.getHandler(handlerno).getWriteRetries() + "
		// retries)\r\n";
	...
	public void syncStorage() {
		if ((this.isInOp()) && (this.syncSkipped < DWDefs.DISK_MAX_SYNC_SKIPS)) {
			logger.debug("Ignoring sync request because we are processing a protocol operation ("
					+ (this.syncSkipped + 1) + " of " + DWDefs.DISK_MAX_SYNC_SKIPS + ")");
			this.syncSkipped++;
