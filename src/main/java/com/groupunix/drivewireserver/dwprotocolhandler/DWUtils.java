...
public class DWUtils {
		...
		if (hexs.length() > 0) {
			if ((hexs.length() % 2) == 0) {
				...
				for (int i = 0; i < (hexs.length() / 2); i++) {
					res[i] = (byte) Integer.parseInt(hexs.substring(i * 2, i * 2 + 2), 16);
				}
	...
	public static boolean isStringFalse(String tf) {
		if (tf == null)
			return false;
		if (tf.equalsIgnoreCase("false") || tf.equalsIgnoreCase("no") || tf.equals("0"))
			return true;
		return false;
	}
	...
	public static boolean isStringTrue(String tf) {
		if (tf == null)
			return false;
		if (tf.equalsIgnoreCase("true") || tf.equalsIgnoreCase("yes") || tf.equals("1"))
			return true;
		return false;
	}
	...
	public static String pretty5ByteDateTime(byte[] data) {
		String res = String.format("%02d:%02d %02d/%02d/%04d", (data[3] & 0xff), (data[4] & 0xff), (data[1] & 0xff),
				(data[2] & 0xff), (1900 + (data[0] & 0xff)));
	...
	public static String chooseSerialPort(String currentPort, int handlerno) {
		SerialPort[] ports = SerialPort.getCommPorts();
		if (ports.length == 0) {
			System.out.println("No serial ports found on this system.");
			return null;
		}

		System.out.println("\n--- Serial Port Selection for Handler #" + handlerno + " ---");
		System.out.println("Configured port '" + currentPort + "' was not found or could not be opened.");
		System.out.println("Available serial ports:");
		for (int i = 0; i < ports.length; i++) {
			System.out
					.println(i + ": " + ports[i].getSystemPortName() + " (" + ports[i].getDescriptivePortName() + ")");
		}
