...
public class DWLogAppender extends AppenderSkeleton {
	...
	protected void append(LoggingEvent event) {
		// fatal to console
		if ((event.getLevel() == Level.FATAL) && (!DriveWireServer.isConsoleLogging())
				&& (!DriveWireServer.isDebug())) {

			System.out.println("FATAL: " + event.getRenderedMessage());
		}

		// ignore those pesky XMLConfiguration debug messages and massive httpd client
		// noise
		if ((event != null) && (event.getMessage() != null) && (event.getLocationInformation().getClassName() != null)
				&& (!event.getMessage().equals("ConfigurationUtils.locate(): base is null, name is null"))
				&& (!event.getLocationInformation().getClassName().startsWith("org.apache.commons.httpclient"))) {
