...
public class DWVSerialPorts {
			...
			try {
				...
				if (dwProto.getConfig().containsKey("MIDIDefaultOutput")) {
					...
					if ((devno < 0) || (devno > infos.length)) {
						logger.warn(
								"Invalid MIDI output device # " + devno + " specified in MIDIDefaultOutput setting");
					} else {
	...
	public byte[] serRead() {
		...
		if (oldest1port > -1) {
			...
			// logger.debug("SERREADM RESPONSE: " + Integer.toBinaryString(response[0]) + "
			// " + Integer.toBinaryString(response[1]));
		...
		// logger.debug("SERREAD RESPONSE: " + Integer.toBinaryString(response[0]) + " "
		// + Integer.toBinaryString(response[1]));
		...
		if ((port < this.maxports) && (port >= 0)) {
			if (vserialPorts[port] != null) {
				if (vserialPorts[port].isOpen()) {
					...
					throw new DWPortNotOpenException("Port " + port + " is not open (but coco sent us a byte: "
							+ (0xff & databyte) + " '" + Character.toString((char) databyte) + "')");
				}
			} else {
	...
	public void setMIDIDevice(MidiDevice device) throws MidiUnavailableException, IllegalArgumentException {
		...
		DriveWireServer.submitMIDIEvent(this.dwProto.getHandlerNo(), "device",
	...
	public int getGMInstrument(int voice) {
		...
		for (Iterator<HierarchicalConfiguration> it = mappings.iterator(); it.hasNext();) {
			...
			if ((sub.getInt("[@dev]") + this.midiProfConf.getInt("[@dev_adjust]", 0)) == voice) {
				...
				logger.debug("MIDI: profile '" + this.midiProfConf.getString("[@name]") + "' translates device inst "
						+ voice + " to GM instr " + xvoice);
				return (xvoice);
			}
