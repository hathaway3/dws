		...
		try {
			...
			if (dwProto.getConfig().containsKey("ListenAddress")) {
				srvr = new ServerSocket(this.tcpport, BACKLOG,
						InetAddress.getByName(dwProto.getConfig().getString("ListenAddress")));
			} else {
				srvr = new ServerSocket(this.tcpport, BACKLOG);
			}
		...
		while ((wanttodie == false) && (!srvr.isClosed())) {
			...
			if (this.connthread != null) {
				if (this.connthread.isAlive()) {
					...
					try {
						skt.getOutputStream()
								.write(("The term device is already connected to a session (from " + this.dwVSerialPorts
										.getListenerPool().getConn(conno).socket().getInetAddress().getHostName()
										+ ")\r\n").getBytes());
						skt.close();
					} catch (IOException e) {
