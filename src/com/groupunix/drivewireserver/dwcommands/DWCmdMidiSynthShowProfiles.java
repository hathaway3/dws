...
public class DWCmdMidiSynthShowProfiles extends DWCommand {
	...
	public DWCommandResponse parse(String cmdline) {
		...
		List<HierarchicalConfiguration> profiles = DriveWireServer.getConfigurationsAt("midisynthprofile");

		for (Iterator<HierarchicalConfiguration> it = profiles.iterator(); it.hasNext();) {
			HierarchicalConfiguration mprof = it.next();

			text += String.format("%-10s: %-35s dev_adjust: %2d  gm_adjust: %2d", mprof.getString("[@name]", "n/a"),
					mprof.getString("[@desc]", "n/a"), mprof.getInt("[@dev_adjust]", 0),
					mprof.getInt("[@gm_adjust]", 0));
			text += "\r\n";
		}
