...
public class DWUIClientThread implements Runnable {
	...
	private void doCmd(String cmd) throws IOException {
		...
		// malformed command
		if (div < 1) {
			sendUIresponse(new DWCommandResponse(false, DWDefs.RC_UI_MALFORMED_REQUEST,
		...
		while (!DriveWireServer.isReady() && (waits < DWDefs.UITHREAD_SERVER_WAIT_TIME)) {
			try {
				...
				sendUIresponse(new DWCommandResponse(false, DWDefs.RC_SERVER_NOT_READY,
		...
		/*
		 * if (this.instance > -1) { waits = 0; while
		 * (!DriveWireServer.getHandler(instance).isReady() && (waits <
		 * DWDefs.UITHREAD_INSTANCE_WAIT_TIME)) { try {
		 * Thread.sleep(DWDefs.UITHREAD_WAIT_TICK); waits += DWDefs.UITHREAD_WAIT_TICK;
		 * } catch (InterruptedException e) {
		 * logger.warn("Interrupted while waiting for instance ready");
		 * sendUIresponse(new DWCommandResponse(false, DWDefs.RC_INSTANCE_NOT_READY,
		 * "Interrupted while waiting for instance ready")); return; } }
		 * 
		 * if (!DriveWireServer.getHandler(instance).isReady()) {
		 * logger.warn("Timed out waiting for instance #" + instance + " to be ready");
		 * sendUIresponse(new DWCommandResponse(false, DWDefs.RC_INSTANCE_NOT_READY,
		 * "Timed out waiting for instance #" + instance + " to be ready")); return; } }
		 */
	...
	private void sendXresponse(XCommandResponse resp) throws IOException {
		if (DriveWireServer.getLogUIConnections()) {
			if (resp.getResponseCode() == 0)
				...
				logger.debug("X command failed: #" + (0xFF & resp.getResponseCode()) + ": "
						+ DWUtils.byteArrayToHexString(resp.getResponseBytes()));
		}
