...
public class DiskStatusWin {
	...
	public DiskStatusWin(DiskDef cdisk, int x, int y)
	{
		...
		background = org.eclipse.wb.swt.SWTResourceManager.getImage(DiskStatusWin.class, "/disk/driveinfo_bg.png");
		
		
		
		if (DiskStatusWin.fontDiskNumber == null)
		{
			HashMap<String,Integer> fontmap = new HashMap<String,Integer>();
			
			fontmap.put("Droid Sans", SWT.BOLD);
			DiskStatusWin.fontDiskNumber = UIUtils.findFont(this.display, fontmap, "255", 81, 57);
		}
		
		
		if (DiskStatusWin.fontDiskGraph == null)
		{
			HashMap<String,Integer> fontmap = new HashMap<String,Integer>();
			
			fontmap.put("Droid Sans", SWT.NORMAL);
			DiskStatusWin.fontDiskGraph = UIUtils.findFont(this.display, fontmap, "255", 20, 15);
		}
	}
	...
	private void createDiskStatusItems()
	{
		...
		getDiskStatusLED().setBackground(DiskStatusWin.colorDiskBG);
		...
		getDiskGraph().setBackground(DiskStatusWin.colorDiskBG);
		...
		getDiskGraph().addPaintListener(new PaintListener()
		{
		   public void paintControl(final PaintEvent event)
		   {
			   ...
			   {
				  event.gc.setBackground(DiskStatusWin.colorDiskBG);
				  event.gc.fillRectangle(0,0, DiskStatusWin.DGRAPH_WIDTH, 52);
			   }
	...
	protected void createContents() 
	{
		...
		shlDwDrive.setImage(SWTResourceManager.getImage(DiskStatusWin.class, "/dw/dw4square.jpg"));
		...
		spacer.setBounds(background.getImageData().width, background.getImageData().height,0,0);
			...
			getCompositeDisk().addPaintListener(new PaintListener() {
				public void paintControl(PaintEvent e) 
				{
					...
					e.gc.drawImage(DiskStatusWin.background, 0, 0);
					
					// disk #
					e.gc.setTextAntialias(SWT.ON);
					e.gc.setAntialias(SWT.ON);
					e.gc.setFont(DiskStatusWin.fontDiskNumber); 
					
					if (currentDisk.isLoaded())
					{
						e.gc.setForeground(DiskStatusWin.colorShadow); 
						e.gc.drawText(currentDisk.getDriveNo()+"",75,138,true);
					}
										
					e.gc.setForeground(DiskStatusWin.colorDiskFG); 
					...
					if (currentDisk.isLoaded())
					{
						e.gc.drawImage(org.eclipse.wb.swt.SWTResourceManager.getImage(DiskStatusWin.class, "/disk/disk_lever_inserted.png"), 320, 152);
					}
					else
					{
						e.gc.drawImage(org.eclipse.wb.swt.SWTResourceManager.getImage(DiskStatusWin.class, "/disk/disk_lever_ejected.png"), 320, 152);
			...
			btnDiskFile.setBackground(DiskStatusWin.colorDiskBG);
			btnDiskFile.setText("...");
